...
export function useBlogDetail() {
    ...
    const fetchBlogDetail = async (slug) => {
        if (!slug) {
            error.value = {
                ...
                status: null,
                originalError: 'Missing slug parameter'
            }
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL_REMEDIS}api/client/blog/${slug}`, {
                method: 'GET',
                headers: {
            ...
            if (!response.ok) {
                ...
                switch (response.status) {
                    ...
                    case 401:
                        ...
                        break
                    case 403:
                        errorMessage = 'Akses ditolak. Anda tidak memiliki izin untuk mengakses artikel ini.'
                        errorType = 'FORBIDDEN'
                        break
                    case 404:
            ...
            console.error('Blog Detail Fetch Error:', {
                ...
                status: statusCode,
                originalError: err.message,
                url: `${import.meta.env.VITE_API_URL_REMEDIS}api/client/blog/${slug}`
            })
    ...
    const retryFetch = (slug) => {
        return fetchBlogDetail(slug)
    }
